...
function deleteXml() returns error? {
    ...
    foreach int currentRetry in 1 ..< maxIterations + 1 {
        ...
        io:println("deleteXml: " + contacts.toString());
    ...
    //get batch request
    foreach int currentRetry in 1 ..< maxIterations + 1 {
        error|json|xml|string batchRequest = baseClient->getBatchRequest(deleteJob, batchId);
    ...
    //get batch result
    foreach int currentRetry in 1 ..< maxIterations + 1 {
        error|json|xml|string|Result[] batchResult = baseClient->getBatchResult(deleteJob, batchId);
