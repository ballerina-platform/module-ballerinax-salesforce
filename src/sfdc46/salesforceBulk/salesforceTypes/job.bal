...
function createJobRecordFromJson(json jobDetails) returns Job | SalesforceError {
    ...
    if (job is Job) {
        json | error extIdField = jobDetails.externalIdFieldName;
        if (extIdField is json) {
            job.externalIdFieldName = extIdField.toString();
        }
        json | error assignmentRuleId = jobDetails.assignmentRuleId;
        if (jobDetails.assignmentRuleId is json) { 
            job.assignmentRuleId = assignmentRuleId.toString();
        }
        return job;
    } else {
        SalesforceError sfError = {
