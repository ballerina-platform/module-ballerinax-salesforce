...
public type SalesforceConnector client object {
    public http:Client salesforceClient;

    public function __init(string baseUrl, SalesforceConfiguration salesforceConfig) {
        self.salesforceClient = new(baseUrl, config = salesforceConfig.clientConfig);
    }
...
remote function SalesforceConnector.getAvailableApiVersions() returns json|SalesforceConnectorError {
    string path = prepareUrl([BASE_PATH]);
    return self->getRecord(path);
}

remote function SalesforceConnector.getResourcesByApiVersion(string apiVersion) returns json|SalesforceConnectorError {
    string path = prepareUrl([BASE_PATH, apiVersion]);
    return self->getRecord(path);
}

remote function SalesforceConnector.getOrganizationLimits() returns json|SalesforceConnectorError {
...
remote function SalesforceConnector.getQueryResult(string receivedQuery)
                                         returns json|SalesforceConnectorError {
    string path = prepareQueryUrl([API_BASE_PATH, QUERY], [Q], [receivedQuery]);
    return self->getRecord(path);
}

remote function SalesforceConnector.getNextQueryResult(string nextRecordsUrl)
                                         returns json|SalesforceConnectorError {
    return self->getRecord(nextRecordsUrl);
}

remote function SalesforceConnector.getAllQueries(string queryString) returns json|SalesforceConnectorError {
    string path = prepareQueryUrl([API_BASE_PATH, QUERYALL], [Q], [queryString]);
    return self->getRecord(path);
}

remote function SalesforceConnector.explainQueryOrReportOrListview(string queryReportOrListview)
                                         returns json|SalesforceConnectorError {
...
remote function SalesforceConnector.searchSOSLString(string searchString) returns json|SalesforceConnectorError {
...
remote function SalesforceConnector.getAccountById(string accountId) returns json|SalesforceConnectorError {
    string path = prepareUrl([API_BASE_PATH, SOBJECTS, ACCOUNT, accountId]);
    return self->getRecord(path);
}

remote function SalesforceConnector.createAccount(json accountRecord) returns string|SalesforceConnectorError {
    return self->createRecord(ACCOUNT, accountRecord);
}

remote function SalesforceConnector.updateAccount(string accountId, json accountRecord)
                                         returns boolean|SalesforceConnectorError {
    return self->updateRecord(ACCOUNT, accountId, accountRecord);
}

remote function SalesforceConnector.deleteAccount(string accountId) returns boolean|SalesforceConnectorError {
    return self->deleteRecord(ACCOUNT, accountId);
}

// ============================ LEAD SObject: get, create, update, delete ===================== //

remote function SalesforceConnector.getLeadById(string leadId) returns json|SalesforceConnectorError {
    string path = prepareUrl([API_BASE_PATH, SOBJECTS, LEAD, leadId]);
    return self->getRecord(path);
}

remote function SalesforceConnector.createLead(json leadRecord) returns string|SalesforceConnectorError {
    return self->createRecord(LEAD, leadRecord);
}

remote function SalesforceConnector.updateLead(string leadId, json leadRecord)
                                         returns boolean|SalesforceConnectorError {
    return self->updateRecord(LEAD, leadId, leadRecord);
}

remote function SalesforceConnector.deleteLead(string leadId) returns boolean|SalesforceConnectorError {
    return self->deleteRecord(LEAD, leadId);
}

// ============================ CONTACTS SObject: get, create, update, delete ===================== //

remote function SalesforceConnector.getContactById(string contactId) returns json|SalesforceConnectorError {
    string path = prepareUrl([API_BASE_PATH, SOBJECTS, CONTACT, contactId]);
    return self->getRecord(path);
}

remote function SalesforceConnector.createContact(json contactRecord) returns string|SalesforceConnectorError {
    return self->createRecord(CONTACT, contactRecord);
}

remote function SalesforceConnector.updateContact(string contactId, json contactRecord)
                                         returns boolean|SalesforceConnectorError {
    return self->updateRecord(CONTACT, contactId, contactRecord);
}

remote function SalesforceConnector.deleteContact(string contactId) returns boolean|SalesforceConnectorError {
    return self->deleteRecord(CONTACT, contactId);
}

// ============================ OPPORTUNITIES SObject: get, create, update, delete ===================== //

remote function SalesforceConnector.getOpportunityById(string opportunityId) returns json|SalesforceConnectorError {
    string path = prepareUrl([API_BASE_PATH, SOBJECTS, OPPORTUNITY, opportunityId]);
    return self->getRecord(path);
}

remote function SalesforceConnector.createOpportunity(json opportunityRecord) returns string|SalesforceConnectorError {
    return self->createRecord(OPPORTUNITY, opportunityRecord);
}

remote function SalesforceConnector.updateOpportunity(string opportunityId, json opportunityRecord)
                                         returns boolean|SalesforceConnectorError {
    return self->updateRecord(OPPORTUNITY, opportunityId, opportunityRecord);
}

remote function SalesforceConnector.deleteOpportunity(string opportunityId) returns boolean|SalesforceConnectorError {
    return self->deleteRecord(OPPORTUNITY, opportunityId);
}

// ============================ PRODUCTS SObject: get, create, update, delete ===================== //

remote function SalesforceConnector.getProductById(string productId) returns json|SalesforceConnectorError {
    string path = prepareUrl([API_BASE_PATH, SOBJECTS, PRODUCT, productId]);
    return self->getRecord(path);
}

remote function SalesforceConnector.createProduct(json productRecord) returns string|SalesforceConnectorError {
    return self->createRecord(PRODUCT, productRecord);
}

remote function SalesforceConnector.updateProduct(string productId, json productRecord)
                                         returns boolean|SalesforceConnectorError {
    return self->updateRecord(PRODUCT, productId, productRecord);
}

remote function SalesforceConnector.deleteProduct(string productId) returns boolean|SalesforceConnectorError {
    return self->deleteRecord(PRODUCT, productId);
}

//===========================================================================================================//

remote function SalesforceConnector.getFieldValuesFromSObjectRecord(string sObjectName, string id, string fields)
                                         returns json|SalesforceConnectorError {
...
remote function SalesforceConnector.getFieldValuesFromExternalObjectRecord(string externalObjectName, string id,
                                        string fields) returns json|SalesforceConnectorError {
...
remote function SalesforceConnector.createMultipleRecords(string sObjectName, json records)
                                         returns json|SalesforceConnectorError {
...
remote function SalesforceConnector.getRecordByExternalId(string sObjectName, string fieldName, string fieldValue)
                                         returns json|SalesforceConnectorError {
    string path = prepareUrl([API_BASE_PATH, SOBJECTS, sObjectName, fieldName, fieldValue]);
    return self->getRecord(path);
}

remote function SalesforceConnector.upsertSObjectByExternalId(string sObjectName, string fieldId, string fieldValue,
...
remote function SalesforceConnector.getDeletedRecords(string sObjectName, string startTime, string endTime)
                                         returns json|SalesforceConnectorError {
    string path = prepareQueryUrl([API_BASE_PATH, SOBJECTS, sObjectName, DELETED], [START, END], [startTime, endTime]);
    return self->getRecord(path);
}

remote function SalesforceConnector.getUpdatedRecords(string sObjectName, string startTime, string endTime)
                                         returns json|SalesforceConnectorError {
...
remote function SalesforceConnector.describeAvailableObjects() returns json|SalesforceConnectorError {
    string path = prepareUrl([API_BASE_PATH, SOBJECTS]);
    return self->getRecord(path);
}

remote function SalesforceConnector.getSObjectBasicInfo(string sobjectName) returns json|SalesforceConnectorError {
    string path = prepareUrl([API_BASE_PATH, SOBJECTS, sobjectName]);
    return self->getRecord(path);
}

remote function SalesforceConnector.describeSObject(string sObjectName) returns json|SalesforceConnectorError {
    string path = prepareUrl([API_BASE_PATH, SOBJECTS, sObjectName, DESCRIBE]);
    return self->getRecord(path);
}

remote function SalesforceConnector.sObjectPlatformAction() returns json|SalesforceConnectorError {
...
remote function SalesforceConnector.getRecord(string path) returns json|SalesforceConnectorError {
    http:Response|error response = self.salesforceClient->get(path);
    return checkAndSetErrors(response, true);
}

remote function SalesforceConnector.createRecord(string sObjectName, json recordPayload)
...
remote function SalesforceConnector.updateRecord(string sObjectName, string id, json recordPayload)
                                         returns boolean|SalesforceConnectorError {
    http:Request req = new;
...
remote function SalesforceConnector.deleteRecord(string sObjectName, string id)
                                         returns boolean|SalesforceConnectorError {
    http:Request req = new;
    string path = prepareUrl([API_BASE_PATH, SOBJECTS, sObjectName, id]);
