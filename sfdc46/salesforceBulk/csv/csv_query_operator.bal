...
public type CsvQueryOperator client object {
    ...
    public remote function getResult(string batchId, string resultId) returns string | SalesforceError;
};
...
public remote function CsvQueryOperator.addQuery(string queryString) returns Batch | SalesforceError {
    xml | SalesforceError xmlResponse = self.httpClient->createCsvRecord([JOB, self.job.id, BATCH], queryString);
...
public remote function CsvQueryOperator.getJobInfo() returns Job | SalesforceError {
...
#
# + return - Job record if successful else SalesforceError occured
public remote function CsvQueryOperator.closeJob() returns Job | SalesforceError {
...
#
# + return - Job record if successful else SalesforceError occured
public remote function CsvQueryOperator.abortJob() returns Job | SalesforceError {
...
# + batchId - batch ID 
# + return - Batch record if successful else SalesforceError occured
public remote function CsvQueryOperator.getBatchInfo(string batchId) returns Batch | SalesforceError {
...
public remote function CsvQueryOperator.getAllBatches() returns BatchInfo | SalesforceError {
...
public remote function CsvQueryOperator.getResultList(string batchId) returns ResultList | SalesforceError {
...
public remote function CsvQueryOperator.getResult(string batchId, string resultId) returns string | SalesforceError {
    return self.httpClient->getCsvRecord([JOB, self.job.id, BATCH, batchId, RESULT, resultId]);
}
