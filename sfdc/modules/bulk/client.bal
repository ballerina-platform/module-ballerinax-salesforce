    ...
    {label: "Result"} error|json|xml|string|Result[] {
        ...
        match bulkJob.jobDataType {
            JSON => {
                ...
                if (QUERY == bulkJob.operation) {
                    return getJsonQueryResult(resultResponse, path, self.salesforceClient, self.clientHandler);
                }
            ...
            XML => {
                ...
                if (QUERY == bulkJob.operation) {
                    return getXmlQueryResult(resultResponse, path, self.salesforceClient, self.clientHandler);
                }
            ...
            CSV => {
                if (QUERY == bulkJob.operation) {
                    xml resultResponse = check checkXmlPayloadAndSetErrors(response);
                    return getCsvQueryResult(resultResponse, path, self.salesforceClient, self.clientHandler);
                }
