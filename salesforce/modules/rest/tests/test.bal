...
function testCreateRecordForType() {
    log:printInfo("baseClient -> createRecord()");
    string|Error stringResponse = baseClient->createRecordWithType(ACCOUNT, accountRecord);

    if stringResponse is string {
        test:assertNotEquals(stringResponse, "", msg = "Found empty response!");
        testRecordId = stringResponse;
    } else {
        test:assertFail(msg = stringResponse.detail()?.body.toString());
    }
}

@test:Config {
    enable: true,
    dependsOn: [testCreateRecordForType]
}
function testGetRecordWithType() {
    log:printInfo("baseClient -> testGetRecordWithType()");
    string path = "/services/data/v48.0/sobjects/Account/" + testRecordId;
    Account|Error response = baseClient->getRecordWithType(path);

    if response is Account {
        test:assertNotEquals(response, (), msg = "Found null JSON response!");
        test:assertEquals(response["Name"], "KING Holdings", msg = "Name key mismatched in response");
        test:assertEquals(response["BillingCity"], "Colombo 3", msg = "BillingCity key mismatched in response");
    } else {
        test:assertFail(msg = response.detail()?.body.toString());
    }
}
...
function testGetRecordByIdWithType() {
    log:printInfo("baseClient -> testGetRecordByIdWithType()");
    string id = "0015g00000hDa33AAC";
    Account|Error response = baseClient->getRecordByIdWithType("Account", id);

    if response is Account {
        test:assertNotEquals(response, (), msg = "Found null JSON response!");
        test:assertEquals(response["Name"], "Express Logistics and Transport", msg = "Name key mismatched in response");
    } else {
        test:assertFail(msg = response.detail()?.body.toString());
    }
}
...
@test:Config {
    enable: true,
    dependsOn: [testCreateRecordForType, testGetRecordWithType]
}
function updateRecordWithType() {
    ...
    Error? response = baseClient->updateRecordWithType(ACCOUNT, testRecordId, account);
...
function testGetQueryResultStream() returns error? {
    log:printInfo("baseClient -> testGetQueryResultStream()");
    string sampleQuery = "SELECT name FROM Account";
    stream<Account, error?> res = check baseClient->getQueryResultStreamWithType(sampleQuery);
    // error? forEach = res.forEach(function(Account ac) {
    //     log:printInfo(ac.Name.toString());
    // });
}
...
function testSearchSOSLStringStream() returns error? {
    log:printInfo("baseClient -> testSearchSOSLStringStream()");
    string searchString = "FIND {WSO2 Inc}";
    stream<record{}, error?> res = check baseClient->searchSOSLStringStreamWithType(searchString);
    // error? forEach = res.forEach(function(record{} ac) {
    //     log:printInfo(ac.toString());
    // });
}
